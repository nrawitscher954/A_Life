       ...
       // Rigidbody rb;
        ...
        void Update()
        {
            ...
            if (GenerationManager.generationCount != 0)
            {
                ...
                if (SimulationManager.Get().addFixedJoints == true && SimulationManager.Get().addRigidBodyCollider == true && gameObject.GetComponent<SphereCollider>().isTrigger ==true)// if rigid body collider is on and its ignoring physics engine
                {
