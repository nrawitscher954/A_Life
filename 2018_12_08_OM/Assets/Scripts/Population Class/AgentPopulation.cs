       ...
       GameObject generationManager;
        ...
        void Update()
        {
            ...
            if (time == timeToInitFirstAgent&& runCoroutine == false && PeoplePopulation.peopleList.Count != 0)
            {
                ...
                // PickRandomAgentstoActivate(5);


                //MovePopTowardsPixelArrayCntrMass();

                 ActivateAgentBasedOnCntOfPplMass();
          ...
          //  emmiterAgentList = FindObjectsWithTag("SignalEmmiter");
            ...
            //if (deActivatedAgentList.Count < 200 && IntiPopulation == true && deActivatedAgentList.Count != 0)
            //{
            //    if (SimulationManager.Get().is3D)
            //    {
            //        InitializeAgents3DArray(populationCount);
        ...
        public void ActivateAgentsBasedOnInitClusterPositionFromRhino(int num)
        {
         ...
         //   {
            ...
            for (int i = 0; i < KNearest.Count; i++)
            {
                ...
                {
                    KNearest[i].GetComponent<MeshRenderer>().material.color = agent.EnergyColor(SimulationManager.Get().startColorEnergy, SimulationManager.Get().endColorEnergy, agent.energyLevel);
                }

                if (SimulationManager.Get().displayColorByState)
                {
                    KNearest[i].GetComponent<MeshRenderer>().material.color = Color.blue;

                }

                if (SimulationManager.Get().displayColorbyDisplacement)
                {
                    KNearest[i].GetComponent<MeshRenderer>().material.color = Color.grey;
                }
                ...
                if (SimulationManager.Get().GPUInstancing)
                {
                    ...
                    MeshRenderer meshR = KNearest[i].GetComponent<MeshRenderer>();
                    if (meshR)
                    {
        ...
        private void InitializeAgents3DArray(int number)
        {
            for (int i = 0; i < System.Math.Ceiling(Mathf.Pow(number, 1.0f / 3.0f)); i++)
            {
                ...
                for (int j = 0; j < System.Math.Ceiling(Mathf.Pow(number, 1.0f / 3.0f)); j++)
                {
                    ...
                    for (int k = 0; k < System.Math.Ceiling(Mathf.Pow(number, 1.0f / 3.0f)); k++)
                    {
                      ...
                      //  test.Add((Instantiate(agentPrefab, distributionArea, Quaternion.identity)));
            ...
            for (int i = 0; i < populationList.Count; i++)
            {
                ...
                //populationList[i].transform.position -= new Vector3(14.24f, 0, 14.24f);
        ...
        private List<Agent> PickRandomAgentstoActivate(int number)
        {
            ...
            for (int i = 0; i < number; i++)
            {
                ...
                int index = UnityEngine.Random.Range(0, populationCount);
            ...
            for (int i = 0; i < listToReturn.Count; i++)
            {
                ...
                if (SimulationManager.Get().displayColorbyEnergy)
                {
                    listToReturn[i].GetComponent<MeshRenderer>().material.color = agent.EnergyColor(SimulationManager.Get().startColorEnergy, SimulationManager.Get().endColorEnergy,agent.energyLevel);
                }
                ...
                if (SimulationManager.Get().displayColorbyDisplacement)
                {
                    listToReturn[i].GetComponent<MeshRenderer>().material.color = Color.grey;
                }
            }
